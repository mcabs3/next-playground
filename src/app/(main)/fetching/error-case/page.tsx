...
`}
				...
				<blockquote>
					...
					<ul className="mt-2 list-disc space-y-2 pl-6">
						<li>
							<code>error.tsx</code> files create React Error Boundaries at any
							level of your route tree — errors are caught and rendered in place
							without crashing the whole page.
						</li>
						<li>
							Error components must be Client Components and receive a{" "}
							<code>reset()</code> function for recovery.
						</li>
						<li>
							Placement controls blast radius: a narrow error boundary means
							only the failed section is replaced, not the entire page.
						</li>
						<li>
							When using Suspense + Server Components, pair each Suspense
							boundary with an Error Boundary so a single failed fetch doesn't
							take down unrelated content.
						</li>
						<li>
							<code>global-error.tsx</code> is the last resort — it catches
							errors in the root layout and replaces the entire document.
						</li>
					</ul>
				</blockquote>
